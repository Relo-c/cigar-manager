...
<template><section><header class="page-header"><p class="eyebrow">散养</p><h1>{{ inventory.looseStickCount }} 支库存</h1></header><van-search v-model="query" placeholder="搜索品牌、型号、年份或柜号" shape="round" />
  ...
  <van-floating-bubble icon="plus" axis="xy" magnetic="x" @click="open()" /><van-popup v-model:show="showForm" position="bottom" round :style="{height:'78%'}"><div class="sheet-header"><strong>{{ editing?'编辑散养':'新增散养' }}</strong><van-icon name="cross" @click="showForm=false" /></div><LooseForm :model-value="editing" @save="save" /></van-popup>
